...
import {
    ...
    calculateAngleBetweenThreePoints, calculateAngleToTheIntersection, calculateDistanceBetweenLineAndDot,
    calculateDistances,
...
const Diagnostic = () => {
    ...
    useEffect(() => {
        ...
        if (distances.some(dis => typeof dis === "object" && dis !== null) &&
            ...
            const newDifference = ((30 - distances[0].value) / distances[0].value) * 100;
        ...
        if (dots.some(item => item.dotName === "A") && dots.length <= indexA) {
            ...
            const angle = calculateAngleBetweenThreePoints(dots1, dots2, dots3, difference);
            setDistances(prev => [
                ...prev,
                {
                    [`value`]: angle, ["key"]: "deg",
        ...
        if (dots.some(item => item.dotName === "Snp") && dots.length <= indexSnp) {
            ...
            const angle = calculateAngleBetweenFourPoints(dots1, dots2, dots3, dots4);
            const value = calculateAngleToTheIntersection(dots4, dots3, dots5, difference);
            setDistances(prev => [
                ...prev,
                {
                    [`value`]: angle, ["key"]: "deg",
        ...
        if (dots.some(item => item.dotName === "is") && dots.length <= indexIs) {
            ...
            const dots3 = dots.find(obj => obj.dotName === "is");
            const angle = calculateDistanceBetweenLineAndDot(dots1, dots2, dots3, distances[0].value);
            setDistances(prev => [
                ...prev,
                {
                    [`value`]: angle, ["key"]: "mm",
