...
const Diagnostic = () => {
  ...
  useEffect(() => {
    ...
    if (
      dots.some(item => item.dotName === "firstDot") &&
      dots.some(item => item.dotName === "secondDot") &&
      dots.length <= 2
    ) {
      calculateDistances(dots[0], dots[1], difference, setDistances);
    }
    if (
      dots.some(item => item.dotName === "S") &&
      dots.some(item => item.dotName === "Se") &&
      dots.some(item => item.dotName === "N") &&
      dots.length <= 6
    ) {
      const dots1 = dots.find(obj => obj.dotName === "S");
      const dots2 = dots.find(obj => obj.dotName === "Se");
      const dots3 = dots.find(obj => obj.dotName === "N");
              ...
              <div className={style.img}>
                ...
                <img
                  src={
                    fileUrl
                      ? fileUrl
                      : "https://e7.pngegg.com/pngimages/224/805/png-clipart-chart-degree-right-angle-protractor-angle.png"
                  }
                  style={{
                    filter: `invert(${
                      invert ? "1" : "0"
